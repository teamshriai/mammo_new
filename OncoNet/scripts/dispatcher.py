...
def launch_experiment(gpu, flag_string):
    ...
    main_script = os.path.join(script_root, 'main.py')
    cmd = [sys.executable, '-u', main_script] + shlex.split(flag_string) + ['--results_path', results_path]
    env = os.environ.copy()
    env['CUDA_VISIBLE_DEVICES'] = str(gpu)
    print("Launched exp: {}".format(' '.join(cmd)))

    if not os.path.exists(results_path) or args.rerun_experiments:
        log_mode = 'a' if "--resume" in flag_string and not args.rerun_experiments else 'w'
        with open(log_path, log_mode) as log_file:
            subprocess.call(cmd, env=env, cwd=dirname(script_root), stdout=log_file, stderr=subprocess.STDOUT)

    if not os.path.exists(results_path):
        # running this process failed, alert me
        job_fail_msg = EXPERIMENT_CRASH_MSG.format(' '.join(cmd), log_path)
        print(job_fail_msg)
